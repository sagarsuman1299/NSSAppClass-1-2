...
public class Weather_Line : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler
{
    //private Vector2 spriteDim = new Vector2(420.21f, 272.26f);

    private Weather_Manager manager;
    public LineRenderer line;

    public void Awake()
    {
        manager = GameObject.Find("Weather_Manager").GetComponent<Weather_Manager>();
    ...
    public void DragStop(PointerEventData eventData, GameObject qsn, GameObject ans)
    {
        ...
        else if (qsn.GetComponentInChildren<Text>().text == (ans.GetComponent<Text>().text))
        {
            ...
            if (manager.Weather_texts.Contains(ans.GetComponent<Text>().text))
            {
                line.SetPosition(1, Cam(v));
            ...
            {
                manager.Weather_texts.Add(ans.GetComponent<Text>().text);
