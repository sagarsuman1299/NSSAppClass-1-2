...
public class Places_Line : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler
{
    //private Vector2 spriteDim = new Vector2(420.21f, 272.26f);

    private Places manager;
    public LineRenderer line;

    public void Awake()
    {
        manager = GameObject.Find("Place_Manager").GetComponent<Places>();
    ...
    public void DragStop(PointerEventData eventData, GameObject qsn, GameObject ans)
    {
        ...
        else if (qsn.GetComponentInChildren<Text>().text == (ans.GetComponent<Text>().text))
        {
            ...
            if (manager.texts.Contains(ans.GetComponent<Text>().text))
            {
                line.SetPosition(1, Cam(v));
            ...
            {
                manager.texts.Add(ans.GetComponent<Text>().text);
